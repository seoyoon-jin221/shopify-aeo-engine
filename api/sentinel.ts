...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { shopDomain } = req.query as { shopDomain?: string } || req.body || {};
    if (!shopDomain) {
      return res.status(400).json({ status: 'BAD_REQUEST', message: 'Missing shopDomain in request body or query' });
    }
    ...
    const category = 'ecommerce'; // arbitrary category
    const queries = [
